...
```bash
py tools/mailbox-attach.py --from wiki@DESKTOP --to wiki@LAPTOP \
    --body "snapshot" --files C:/tmp/wiki.zip
```
詳見 `mailbox-attach.py --help`。**注意**：別跟 `tools/mailbox-discord-file.py`（推檔到 Discord DM 的，port 1904）搞混 — 那個 2026-05-23 改名了，這個 `tools/mailbox-attach.py` 才是 cross-device peer ↔ peer 的。
...
**手動 CLI**：
```bash
py tools/mailbox-retention.py --stats      # 看 disk / 訊息數 / oldest age
py tools/mailbox-retention.py --dry-run    # 報告會刪什麼，不真刪
py tools/mailbox-retention.py --once       # 立即跑一次 sweep
```
...
**手動 CLI**：
```bash
py tools/mailbox-backup.py --stats                     # last_backup_at / count / total bytes
py tools/mailbox-backup.py --list                      # 列現存 snapshot，新→舊
py tools/mailbox-backup.py --once                      # 立刻打一份 + rolling prune
...
**手動 CLI**：
```bash
py tools/mailbox-audit.py --tail                  # 最近 50 筆
py tools/mailbox-audit.py --tail --limit 200
py tools/mailbox-audit.py --since 1h              # 相對時間：15m / 1h / 24h / 7d
py tools/mailbox-audit.py --actor wiki            # 只看某 actor
py tools/mailbox-audit.py --action send           # 只看某動作
py tools/mailbox-audit.py --actor wiki --action send --since 1h
py tools/mailbox-audit.py --stats                 # count + first/last + by_action 分布
py tools/mailbox-audit.py --tail --json           # machine-readable
```
...
```bash
py tools/mailbox-webhooks.py --add my-slack --url https://hooks.slack.com/services/...
...
py tools/mailbox-webhooks.py --deactivate 3   # 不刪、暫停
py tools/mailbox-webhooks.py --delete 3       # 真刪（連 deliveries 一起 CASCADE）
py tools/mailbox-webhooks.py --tail-deliveries --status failed   # debug
...
**手動 CLI**：
```bash
py tools/mailbox-rate-limit.py --stats        # 全局 stats
py tools/mailbox-rate-limit.py --top          # 5-min 最忙 20 scopes
py tools/mailbox-rate-limit.py --reset from:wiki   # 重設某 scope
py tools/mailbox-rate-limit.py --prune        # 砍 >1hr 老 bucket
```
...
- `tools/mailbox-discord-file.py` — CLI，推檔到使用者 Discord DM（port 1904 bridge），**走 Discord REST API**。跟下面 `tools/mailbox-attach.py` 不同：那個是 agent ↔ agent，這個是 agent → Discord user。
- `tools/mailbox-attach.py` — CLI，cross-device 寄訊息+檔案到 peer agent mailbox（port 1905 hub server）。MCP `send(files=[...])` 的 shell 等價物。
- `tools/mailbox-retention.py` — CLI，手動 trigger retention sweep / 看 stats / dry-run（hub-only）
- `tools/mailbox-backup.py` — CLI，手動打 backup / list / restore / stats（hub-only）
